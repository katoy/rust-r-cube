...
mod tests {
    ...
    fn test_solver_extra_coverage() {
        ...
        let _ = solve(&cube1, 1, false);

        // to hit line 181 (break on empty), although best_moves.is_empty() usually caught earlier
        // we can try a 0-move solve inside the loop
        // solve_with_progress
        let (tx, _) = std::sync::mpsc::channel();
        let _ = solve_with_progress(&cube, 1, false, Some(tx));
    }
}
