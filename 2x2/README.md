...
- **テスト網羅率**: コアロジック（`src/cube/` および `src/solver.rs`）において、高いコードカバレッジを維持しています。

| File                     | Regions | Functions |  Lines  |
| :----------------------- | :-----: | :-------: | :-----: |
| `src/cube/enums.rs`      | 100.00% |  100.00%  | 100.00% |
| `src/cube/io.rs`         | 95.33%  |  87.50%   | 97.80%  |
| `src/cube/mod.rs`        | 93.84%  |  85.71%   | 92.63%  |
| `src/cube/rotation.rs`   | 100.00% |  100.00%  | 100.00% |
| `src/cube/validation.rs` | 54.55%  |  33.33%   | 65.71%* |
| `src/solver.rs`          | 97.29%  |  100.00%  | 97.12%  |

> *注: `validation.rs` は機能改修中のため一部ロジックを無効化しており、カバレッジが一時的に低下しています。*
