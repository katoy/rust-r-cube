...
- **双方向BFSのキュー選択最適化**: 前方向（forward）と後ろ方向（backward）のキューサイズを比較し、常にノード数が少ない方のキューから次の層を展開するように最適化。不要なノード展開を大幅に削減し探索効率を最大化。
- **ハイブリッド並列展開（Rayon/シングルスレッド）**: 展開するノード数が少ない場合（閾値1,000未満）はスレッド切り替え・同期のオーバーヘッドを避けるためシングルスレッドで処理し、ノード数が多い場合のみ `Rayon` による並列展開を自動選択するハイブリッド制御を導入。
...
- **並列展開（Rayon）の中間アロケーション削減**: 各探索層の展開時に `flat_map_iter` を用いて平坦な単一 `Vec` にアロケーションを統合し、一時ベクタ生成のオーバーヘッドを解消
...
- **親状態（`Cube`）の保持廃止による超省メモリ化**: 探索履歴マップ `StateMap` の値から親の `Cube`（48バイト）を排除し、直前の移動操作 `Option<Move>`（1バイト）のみを格納。経路復元時には適用された操作の逆操作（`apply_move(mv.inverse())`）で親状態をオンデマンド復元。ハッシュマップのメモリ消費量を劇的に削減し、CPUキャッシュ効率とRehash効率を極限まで向上。
