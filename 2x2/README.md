...
2x2ルービックキューブの**神の数は11**です。これは、どんな状態からでも最大11手で解けることが数学的に証明されています。

### 難易度の例

| パターン     | 必要な手数 | 説明                                             |
| ------------ | ---------- | ------------------------------------------------ |
| R U × 5回    | 10手       | `R U R U R U R U R U` - 最も難しいパターンの一つ |
| R U × 7-8回  | 0手        | 完成状態に戻る（周期性）                         |
| ランダム状態 | 平均8-9手  | 典型的なランダムスクランブル                     |
| 最遠状態     | **11手**   | 神の数に到達する状態（稀）                       |

#### 11手必要な状態の例

以下は、ランダム探索で発見された11手必要な最遠状態の一例です：

**スクランブル手順:**
```
D F D U' F' U' B R' U' F D' B' R U R F' R' R R' U U'
```
...
この状態から完成状態に戻すには、最短で11手が必要です。

### R U パターンの周期性

`R U`（右面回転 → 上面回転）を繰り返すと、興味深い周期性が現れます：

- **1-6回**: 徐々に難しくなる（2手 → 10手）
- **7-8回**: 完成状態に戻る！
- **9-10回**: 再び難しくなる（10手）
- **11-12回**: 簡単な状態に戻る（8手 → 6手）
