...
- **解決設定**:
  - **向き無視**: 各面の色さえ揃えば完成とみなします
  - **向きも揃える**: 色に加えて、ステッカーの矢印まで全て初期状態に揃えます
...
- **メモリ最適化**: 各探索ノードで操作履歴を保持せず、「親への参照」のみを保持することでメモリアロケーションを最小化。
...
| `src/solver.rs`  |  97.32%   |   100.00%   |  96.30%   |
| **Total (Core)** | **99.2%** | **100.00%** | **98.8%** |
