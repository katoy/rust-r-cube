...
export function mount() {
  ...
  <dialog id="help-dialog"><div class="dialog-header"><h2>ひと回しずつ、使ってみよう。</h2><button id="help-close" class="icon-button" aria-label="使い方を閉じる">${icon("close")}</button></div><ol class="help-list"><li><strong>キューブを準備</strong><p>スクランブルで試すか、実物の6面の色を入力します。手順を入力して状態をつくることもできます。</p></li><li><strong>解法を探す</strong><p>探索はブラウザ内で完結します。配色は外部に送信されません。探索中も視点を動かせます。</p></li><li><strong>手順をたどる</strong><p>再生ボタンで自動再生、左右ボタンで1手ずつ確認。回転記号を選ぶと、その手の直後に移動します。</p></li></ol><p class="help-note">U＝上、R＝右、F＝前、D＝下、L＝左、B＝後ろ。′ は逆回転、2 は180°です。時計回りは、回す面を正面から見た向きです。</p><p class="help-note">キーボード：U R F D L B で回転、Shiftで逆回転。Spaceで再生／停止、← →で前後の手順、Home / End で最初 / 最後にジャンプ。</p><p class="help-note">6色3×3に対応しています。センターの矢印方向は色入力画面で指定できます。高速な2段階探索を使用し、最短解は保証しません。</p></dialog>`;
}
export function net(
  host: HTMLElement,
  state: string,
