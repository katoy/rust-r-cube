...
export function mount() {
  ...
  <main>
    ...
    <div class="workspace">
      <section class="stage" aria-label="キューブの操作">
        ...
        <div class="manual-controls"><div class="control-label">ひと回しする <span>SHIFT で逆回転</span></div><div id="face-buttons">${[...FACES].map((f) => `<button class="move-button" data-move="${f}" aria-label="${FACE_NAMES[f]}を回す">${f}</button>`).join("")}<button id="prime" class="modifier" aria-pressed="false" aria-label="逆回転">′</button><button id="double" class="modifier" aria-pressed="false" aria-label="180度回転">2</button></div><div class="history-buttons"><button id="undo" class="icon-button" aria-label="元に戻す">${icon("undo")}</button><button id="redo" class="icon-button" aria-label="やり直す">${icon("redo")}</button><span></span><button id="reset" class="text-button">リセット</button></div></div>
      </section>
      <aside class="panel">
        ...
        <div class="panel-section solution"><div class="section-title"><span class="step-index">03</span><h2>ひと回しずつ、揃える</h2><button id="copy" class="icon-button" aria-label="解法をコピー" disabled>${icon("copy")}</button></div>
          ...
          <div id="solution-content" hidden><div class="solution-stats"><strong id="move-count"></strong><span id="solve-time"></span></div><div id="move-list" aria-label="解法の各ステップ"></div><div class="next-move"><span id="next-symbol"></span><p id="next-instruction"></p></div><div class="playback"><button id="prev" class="icon-button" aria-label="前の1手">${icon("back")}</button><button id="play" class="play-button" aria-label="自動再生">${icon("play")}</button><button id="next" class="icon-button" aria-label="次の1手">${icon("next")}</button><span id="step-count"></span><label class="speed-label">速度<select id="speed" aria-label="再生速度"><option value="1000">0.5×</option><option value="500" selected>1×</option><option value="250">2×</option></select></label></div><input id="timeline" type="range" min="0" value="0" aria-label="再生位置" /></div>
        </div>
      </aside>
    </div>
  ...
  <dialog id="camera-editor"><div class="dialog-header"><div><p class="eyebrow">SCAN YOUR CUBE</p><h2>2方向の画像から入力</h2></div><button id="camera-close" class="icon-button" aria-label="画像入力を閉じる">${icon("close")}</button></div><p>対角に近い2方向から撮影し、各画像の3面を順番に指定します。</p><div class="camera-files"><label>画像A<input id="camera-file-a" type="file" accept="image/*" capture="environment"></label><label>画像B<input id="camera-file-b" type="file" accept="image/*" capture="environment"></label></div><label>読み取る面<select id="camera-face"><option value="U">U・上面</option><option value="R">R・右面</option><option value="F">F・前面</option><option value="D">D・下面</option><option value="L">L・左面</option><option value="B">B・背面</option></select></label><canvas id="camera-canvas" width="640" height="480"></canvas><p id="camera-help" role="status">画像を選び、面の四隅を左上→右上→右下→左下の順でクリックしてください。</p><button id="camera-capture" class="secondary" disabled>この面を読み取る</button><p id="camera-error" role="status"></p><div class="dialog-footer"><span id="camera-progress">0 / 6 面</span><button id="camera-apply" class="primary" disabled>色入力へ反映 ${icon("arrow")}</button></div></dialog>
