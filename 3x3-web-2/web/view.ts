...
export function mount() {
  ...
  <main>
    ...
    <div class="workspace-footer"><span>${icon("check")} 解法は、実際に揃うことを検証してから表示します。</span><div><button id="share-link" class="text-button">${icon("share")} 共有リンク</button><button id="save" class="text-button">${icon("download")} 保存</button><button id="load" class="text-button">${icon("upload")} 読込</button><input id="file" type="file" accept=".json,application/json" hidden /></div></div>
