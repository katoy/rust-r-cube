...
![Core Coverage](https://img.shields.io/badge/core_coverage-99.59%25-brightgreen)
...
### 2x2 ソルバーとの関係（高度な上位拡張）

本リポジトリに搭載されている 3x3 ソルバーは、2x2 ソルバーの基本的な設計思想（座標変換・枝刈りテーブル・IDA* 探索）を踏襲しつつ、3x3 特有の広大な探索空間と物理仕様に対応するために、アルゴリズムを高度に拡張した構造になっています。

#### 1. 設計思想の共通性（2x2 ソルバーの基盤）
- **インデックス化（座標化）**: キューブのコーナーやエッジの配置・ねじれ状態を数値（座標）に変換して計算。
- **Pruning Table (枝刈りテーブル) の利用**: 起動時に目標状態からの最短距離をBFSで事前計算し、ヒューリスティック $h$ として使用。
- **IDA* (反復深化A*) 探索**: 枝刈りテーブルをベースに、動的メモリアロケーションを伴わないDFS（スタックベース）で最短解を探索するコアエンジン。
- **解決状態（24方位）キャッシュ**: `OnceLock` による解決状態キャッシュを用いて、物理的な回転による解決状態の整合性を取る仕組み。

#### 2. 3x3 で拡張された高度な要素
- **探索空間の爆大化に伴う2段階分割 (Kociemba's Two-Phase)**:
  2x2（約 $3.6 \times 10^6$ 状態）とは異なり、3x3の探索空間（約 $4.3 \times 10^{19}$ 状態）は巨大なため、探索を「Phase 1（向き・スライスエッジの位置の解決）」と「Phase 2（置換位置の解決）」の2つのフェーズに分解し、それぞれで IDA* 探索を行うように拡張されています。
- **スーパーキューブ（センター方位）解決 ([solver/fix.rs](./src/solver/fix.rs))**:
  2x2 には存在しない「センターパーツのねじれ（方位）」を解決するため、Kociemba アルゴリズムで全体の配置を揃えた後に、センターパーツのズレを計算して他の配置を崩さないコミュテータ手順を追加する「方位修正フェーズ」がオーケストレーションされています。
- **ランダムセットアップ探索 (Setup-Move Search)**:
  巨大な探索空間から極限まで短い手数を瞬時に引き出すために、開始状態に対して事前にランダムな数手（Setup Move）を加えてからソルバーを実行し、最後にその逆操作を組み合わせるメタ探索レイヤーが実装されています。
...
| `src/cube/validation.rs` | 113      | **100.00%** ✨ |
| `src/cube/piece.rs`      | 174      | **100.00%** ✨ |
